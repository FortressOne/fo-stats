...
# PS  *.JSON:- foreach ($f in (gci 'x:\stats\*.json')) { & .\FO_stats_v1g.ps1 -StatFile ($f.ToString() -replace '\[','`[' -replace '\]','`]') -TextOnly }
###
...
foreach ($jsonFile in $inputFile) {
  ...
  if (!$TextOnly) {
    ...
    $tableHeader += "<tr><th $($columStyle)>Weapon</th><th>Class</th><th $($columStyle)>Kills</th><th $($columStyle)>Dmg</th><th $($columStyle)>Dth</th><th $($columStyle)>DmgT</th><th $($columStyle)>Kills</th><th $($columStyle)>Dmg</th><th $($columStyle)>Dth</th><th $($columStyle)>DmgT</th><th $($columStyle)>Kills</th><th $($columStyle)>Dmg</th><th $($columStyle)>Dth</th><th $($columStyle)>DmgT</th></tr>`n"
  ...
  foreach ($p in $playerList) {
    ...
    if ($aod -ne '') {    
      if ($aod -eq 'Att')     { $refSummary = ([ref]$arrSummaryAttTable); $refClass = ([ref]$arrClassTimeAttTable) }
      elseif ($aod -eq 'Def') { $refSummary = ([ref]$arrSummaryDefTable); $refClass = ([ref]$arrClassTimeDefTable)  }
    ...
    if ($aod -ne '') {    
      ...
      arrSummaryTable-UpdatePlayer -table $refSummary -player $p -kills ([int]$arrFragTotalRnd2.$p) -death ([int]$arrDeathRnd2.$p) -tkill ([int]$arrTKillRnd2.$p)
...
function timeSpan-MinSec {
  ...
  $out = "$($mins):$("{0:d2}" -f $ts.Seconds)"
  return $out
}

foreach ($arr in @($arrSummaryAttTable,$arrSummaryDefTable)) {
  foreach ($p in $arr) {
    $p.FlagTime = timeSpan-MinSec($p.FlagTime)
    $p.TimePlayed = timeSpan-MinSec($p.TimePlayed)
  }
}

foreach ($arr in @($arrClassTimeAttTable,$arrClassTimeDefTable)) {
  foreach ($p in $arr) {
    foreach ($class in @('Sco','Sold','Demo','Med','HwG','Pyro','Spy','Eng')) {
      $p.$class = timeSpan-MinSec($p.$class)
    }
  }
} 

Write-Host "`n"
Write-Host "##############$(if ($jsonFileCount -gt 1) { "##############" })"
Write-Host " FINAL TOTALS $(if ($jsonFileCount -gt 1) { " - $jsonFileCount games" })"
Write-Host "##############$(if ($jsonFileCount -gt 1) { "##############" })`n"
...
$arrClassTimeAttTable | FT Name,Sco,Sold,Demo,Med,HwG,Pyro,Spy,Eng
Write-Host 'Class Time Played - Defence'
$arrClassTimeDefTable | FT Name,Sco,Sold,Demo,Med,HwG,Pyro,Spy,Eng
